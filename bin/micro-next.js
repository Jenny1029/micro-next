...
program
  ...
  .description('create a new project powered by micro-next-cli')
    ...
    const go = async () => {
      try {
        ...
        inquirer
          ...
          .then((answers) => {
            console.log('answers.mainAppTechStack', answers.mainAppTechStack);
            if (projectRoot !== '.') {
              fs.mkdirSync(projectRoot);
            }
            CFonts.say('Micro-Next', {
            ...
            return download(projectRoot, answers.mainAppTechStack).then(
              (target) => {
                return {
                  projectRoot,
                  downloadTemp: target,
